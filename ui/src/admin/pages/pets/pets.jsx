...
() {
  ...
  useEffect(() => {
    fetch("http://localhost:3000/allProducts")
      .then((res) => res.json())
      .then((allProducts) => {
        const petscolllectionCategory = allProducts.find(
          (cat) => cat.categoryId === "pets-collection"
        );

        if (petscolllectionCategory && petscolllectionCategory.products.pets) {
          setTimeout(() => {
            setData(petscolllectionCategory.products.pets);
            setLoading(false);
          }, 500);
        } else {
          setLoading(false);
