...
function Candleholdersadmin() {
  ...
  const fetchData = () => {
    fetch("http://localhost:3000/allProducts")
      ...
      .then((allProducts) => {
        ...
        if (homedecorCategory && homedecorCategory.products.candleholders) {
  ...
  useEffect(() => {
    ...
    const handleProductAdded = (event) => {
      ...
      if (mappedSubcategory === "candle-holders") {
        setData(prevData => {
  ...
  const handleProductUpdated = (updatedProduct, mappedSubcategory) => {
    if (mappedSubcategory === "candleholders") {
      setData(prevData =>
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Home Decor - Candle holders</h2>
...
export default Candleholdersadmin;
