...
function   Doorknobsandhooksadmin() {
  ...
  const fetchData = () => {
    fetch("http://localhost:3000/allProducts")
      ...
      .then((allProducts) => {
        ...
        if (homedecorCategory && homedecorCategory.products.doorknobsandhooks) {
          setData(homedecorCategory.products.doorknobsandhooks);
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    const handleProductAdded = (event) => {
      ...
      if (mappedSubcategory === "door-knobs-and-hooks") {
        setData(prevData => {
  ...
  const handleProductUpdated = (updatedProduct, mappedSubcategory) => {
    if (mappedSubcategory === "doorknobsandhooks") {
      setData(prevData =>
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Home Decor - Door knobs and hooks</h2>
...
export default   Doorknobsandhooksadmin;
