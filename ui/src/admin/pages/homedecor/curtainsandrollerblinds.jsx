...
function  Curtainsandrollerblindsadmin  () {
  ...
  const fetchData = () => {
    fetch("http://localhost:3000/allProducts")
      ...
      .then((allProducts) => {
        ...
        if (homedecorCategory && homedecorCategory.products.curtainsandrollerblinds) {
          setData(homedecorCategory.products.curtainsandrollerblinds);
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    const handleProductAdded = (event) => {
      ...
      if (mappedSubcategory === "curtains-and-roller-blinds") {
        setData(prevData => {
  ...
  const handleProductUpdated = (updatedProduct, mappedSubcategory) => {
    if (mappedSubcategory === "curtainsandrollerblinds") {
      setData(prevData =>
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Home Decor - Curtains and roller blinds</h2>
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {data.map((product) => (
          <AdminProducts
            ...
            pro={product}
            category="home-decor"
            subcategory="curtainsandrollerblinds"
...
export default  Curtainsandrollerblindsadmin  
;
