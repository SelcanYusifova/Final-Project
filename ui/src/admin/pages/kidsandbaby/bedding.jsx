...
function Babybeddingadmin () {
  ...
  const fetchData = () => {
    fetch("http://localhost:3000/allProducts")
      .then((res) => res.json())
      .then((allProducts) => {
        const babyCategory = allProducts.find(
          (cat) => cat.categoryId === "kids-and-baby"
        );

        if (babyCategory && babyCategory.products.babybedding) {
          setData(babyCategory.products.babybedding);
        }
      })
      .catch((error) => {
  ...
  useEffect(() => {
    ...
    const handleProductAdded = (event) => {
      ...
      if (mappedSubcategory === "baby-bedding") {
        setData(prevData => {
  ...
  const handleProductUpdated = (updatedProduct, mappedSubcategory) => {
    if (mappedSubcategory === "babybedding") {
      setData(prevData =>
  ...
  return (
    <div>
      <h2 className="text-2xl font-bold mb-6">Kids and baby - Bedding</h2>
...
export default Babybeddingadmin ;
